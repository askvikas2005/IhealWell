...
class TelleCallerUpdatePatient extends Component {
    constructor(props) {
        super(props)
        this.state = {
            prescriptionModel: {},
    ...
    getAutoAddress = async () => {
        ...
        // let body = { userId: this.state.patient.userId, userType: localStorage.getItem('userType') };
       ...
       //alert(JSON.stringify(body))
        ...
        HttpService.posts(url).post(body).then(({ data }) => {
           // this.setState({ gettingDoctor: false })
            if (data.status === "SUCCESS") {
                if (data.data) {
                  //  alert(JSON.stringify(data.data.firstName))
                  //var splits = data.data.split("$");
    ...
    updateHistory = async (pdetails) => {
        const { cronaPatientId } = this.state
        let url = UrlConstants.baseUrl + UrlConstants.updateHistory
        ...
        body.cronaPatientId= parseInt(cronaPatientId);
        body.hostpitalName=patient.hospitalName;
        body.bedType=patient.bedType;
        body.ambulanceRequired=patient.ambulanceRequired;
        body.ambulanceType=patient.ambulanceType;    
        body.assignedTo=patient.assignedTo;
        body.createdBy=localStorage.getItem('userID');
        body.orderStatus="Review By Tele Caller Doctor";
                ...
                return (
                    <div>
                        <form noValidate autoComplete="off">
                            ...
                            <div>
                                <FormControl
                                    ...
                                    <RadioGroup aria-label="Investigation"
                                        ...
                                        onChange={p => this.setPatientInput('patientStatus', p.target.value)}
                                    >
                                        <FormControlLabel value={'Admitted'} control={<Radio />} label="Yes" />
                                        <FormControlLabel value={'At_Home'} control={<Radio />} label="No" />
                                    </RadioGroup>
                                </FormControl>
                            </div>
                            <div>
                            ...
                            <div className='card' style={{ flexDirection: 'row', alignContent: 'stretch' }}>
                            <FormLabel className='inputStyles' component="legend" style={{ display: 'flex', flexDirection: 'row', backgroundColor: '#154c66', padding: 10, justifyContent: 'center',fontSize: 15, alignSelf: 'center', color: 'white'}}>Tele Caller summary</FormLabel>
                            ...
                            <div style={{ textAlign: 'center' }}>
                                ...
                                <Button type="button" variant="contained" color="primary" onClick={() => !saving && this.updatePatient(patient,'')}>{!saving ? 'Update' : 'Please wait'}</Button>
                            </div>
                            </div> */}
                        </form>
                    </div>
                ...
                {gettingPatient ? <div style={{ textAlign: 'center', margin: '20px' }}>
                   ...
                   <div className='box'>
                       <FormLabel className='inputStyles' component="legend" style={{ display: 'flex', flexDirection: 'row', backgroundColor: '#154c66', padding: 10,fontSize: 15, alignSelf: 'center', color: 'white'}}>
                           Assign To Tele Caller Doctor</FormLabel>
...
export default withStyles(useStyles)(connect(mapStateToprops)(TelleCallerUpdatePatient));
