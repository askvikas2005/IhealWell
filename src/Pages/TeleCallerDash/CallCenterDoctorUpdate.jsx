...
import './CallCenterDoctorUpdate.css'
...
class CallCenterDoctorUpdate extends Component {
    constructor(props) {
        super(props)
        this.state = {
            prescriptionModel: {},
            ...
            availableDoctor: [],
            newDoctor:true,
            gettingDoctor1: false,
            availableDoctor1: [],
            newDoctor1:true,
            gettingDoctor2: false,
            availableDoctor2: [],
            newDoctor2:true,
            patient: {
    ...
    setDoctor1 = async (suggestedByDr1) => {
        await this.setPatientInput('suggestedByDr1', suggestedByDr1);
        await this.setState({ availableDoctor1: [], newDoctor1: false })
    }
    setDoctor2 = async (suggestedByDr2) => {
        await this.setPatientInput('suggestedByDr2', suggestedByDr2);
        await this.setState({ availableDoctor2: [], newDoctor2: false })
    }
    getDoctor = async () => {
        if (this.state.patient?.suggestedByDr?.length < 2) return;
        let url = UrlConstants.baseUrl1 + UrlConstants.getDoctor;
        let body = { userId: this.state.patient.userId, userType: 'TELECALLER' };
    ...
    getDoctor1 = async () => {
        if (this.state.patient?.suggestedByDr1?.length < 2) return;
        let url = UrlConstants.baseUrl1 + UrlConstants.getDoctor;
        let body = { userId: this.state.patient.userId, userType: 'HOSPITAL COORDINATOR' };
        //alert(JSON.stringify(body))
        this.setState({ gettingDoctor1: true });
        HttpService.posts(url).post(body).then(({ data }) => {
           // this.setState({ gettingDoctor: false })
            if (data.status === "SUCCESS") {
                if (data.data) {
                   // alert(JSON.stringify(data.data))
                    this.setState({ availableDoctor1: data.data })
                }
            }
        }).catch((err) => {
            this.setState({ gettingDoctor1: false });
        });
    }
    getDoctor2 = async () => {
        if (this.state.patient?.suggestedByDr2?.length < 2) return;
        let url = UrlConstants.baseUrl1 + UrlConstants.getDoctor;
        let body = { userId: this.state.patient.userId, userType: 'AMBULANCE COORDINATOR' };
        //alert(JSON.stringify(body))
        this.setState({ gettingDoctor2: true });
        HttpService.posts(url).post(body).then(({ data }) => {
           // this.setState({ gettingDoctor: false })
            if (data.status === "SUCCESS") {
                if (data.data) {
                    //alert(JSON.stringify(data.data))
                    this.setState({ availableDoctor2: data.data })
                }
            }
        }).catch((err) => {
            this.setState({ gettingDoctor2: false });
        });
    }
    ...
    updateHistory = async (cpid1) => {
      ...
      const assignID = patient.suggestedByDr.split('$');
       let body = { 
          ...
          updateHistory1 = async (cpid1) => {
            const { cronaPatientId } = this.state
            let url = UrlConstants.baseUrl1 + UrlConstants.updateHistory
            const { patient } = this.state;
          // let body = {...this.state.patient }
          const assignID = patient.suggestedByDr1.split('$');
           let body = { 
                cronaPatientId:parseInt(cpid1),
               // cronaPatientId:parseInt("12354"),
              ...
              if(patient.suggestedByDr1!='')
          {
              ...
              updateHistory2 = async (cpid1) => {
                const { cronaPatientId } = this.state
                let url = UrlConstants.baseUrl1 + UrlConstants.updateHistory
                const { patient } = this.state;
              // let body = {...this.state.patient }
              const assignID = patient.suggestedByDr2.split('$');
               let body = { 
                    cronaPatientId:parseInt(cpid1),
                   // cronaPatientId:parseInt("12354"),
            ...
            if (data.status === "SUCCESS") {
                if (data.data.cronaPatientId) {
                    // this.setState({ cpat : data.data.cronaPatientId.toString() })
                     ...
                     this.updateHistory1(data.data.cronaPatientId)
                     this.updateHistory2(data.data.cronaPatientId)
                       ...
                       <FormLabel className='inputStyles' component="legend" style={{ display: 'flex', flexDirection: 'row', backgroundColor: '#154c66', padding: 10,fontSize: 15, alignSelf: 'center', color: 'white'}}>
                           ...
                           <FormControl
                              ...
                              <AutoTextField 
                               ...
                               <FormLabel className='inputStyles' component="legend" style={{ display: 'flex', flexDirection: 'row', backgroundColor: '#154c66', padding: 10,fontSize: 15, alignSelf: 'center', color: 'white'}}>
                           Assign Hospital Coordinator</FormLabel>
                           <div style={{ display: 'flex', flexDirection: 'row',padding:5,marginLeft:'5'}}>
                           <FormControl
                                            className="inputStyles"
                                            component="fieldset">
                              <AutoTextField 
                                   id={'Assign'}
                                   label={'Assign Hospital Coordinator'}
                                   value={patient.suggestedByDr1}
                                   list={availableDoctor1}
                                   setInput={(v) => this.setPatientInput('suggestedByDr1', v)}
                                   getApi={() => this.getDoctor1()}
                                   showErr={showErr}
                                   setAutoValue={(x) => this.setDoctor1(x)}
                                   clearList={() => this.setState({ availableDoctor1 : [] })}
                                   getting={gettingDoctor1}
                               />
                               </FormControl>
                               
                               </div>
                               <FormLabel className='inputStyles' component="legend" style={{ display: 'flex', flexDirection: 'row', backgroundColor: '#154c66', padding: 10,fontSize: 15, alignSelf: 'center', color: 'white'}}>
                           Assign Ambulance Coordinator</FormLabel>
                           <div style={{ display: 'flex', flexDirection: 'row',padding:5,marginLeft:'5'}}>
                           <FormControl
                                            className="inputStyles"
                                            component="fieldset">
                              <AutoTextField 
                                   id={'Assign'}
                                   label={'Assign Ambulance Coordinator'}
                                   value={patient.suggestedByDr2}
                                   list={availableDoctor2}
                                   setInput={(v) => this.setPatientInput('suggestedByDr2', v)}
                                   getApi={() => this.getDoctor2()}
                                   showErr={showErr}
                                   setAutoValue={(x) => this.setDoctor2(x)}
                                   clearList={() => this.setState({ availableDoctor2 : [] })}
                                   getting={gettingDoctor2}
                               />
                               </FormControl>
...
export default withStyles(useStyles)(connect(mapStateToprops)(CallCenterDoctorUpdate));
