    ...
    function dologin() {
        if (password.length > 0 && username.length > 0) {
            ...
            HttpService.auth(oauthUrl, config).Login().then(res => {
                if (res.status === 200) {
                    let authAllBody = { userLoginID: username.toLowerCase(), accessToken: res.data.value, };
                    ...
                    HttpService.auth(authUrl, configHeader).authAll(authAllBody).then(response => {
                                ...
                                if (authObj.authorities) {
                                    ...
                                    if (authorities) {
                                        if (authorities !== 'ROLE_hcpatient') {
                                            ...
                                            setSpinner(false)
                                            let creadentials = {
                                                userLoginID: username.toLowerCase(), accessToken: res.data.value,
                        ...
                        if (error.response) {
                            ...
                            else { alert('Error,Something went wrong');console.log(error); }
