...
class EnrollPatient extends Component {
    ...
    EnrollPatient = patient => {
        if (!ReduxService.trackreScreenGuard(patient.patientStatus))
            return;
        localStorage.removeItem('fromAddPatient');
        localStorage.setItem('cronaPatientId', `${patient.cronaPatientId}`)
        this.props.history.push('/addpatientcc')
    }
    EnrollPatientHospital = patient => {
        if (!ReduxService.trackreScreenGuard(patient.patientStatus))
            return;
        localStorage.removeItem('fromAddPatient');
        localStorage.setItem('cronaPatientId', `${patient.cronaPatientId}`)
                        ...
                        {(patientList.length > 0 ?
                            patientList.map(p => {
                                return <div
                                    ...
                                    onClick={() => this.EnrollPatientHospital(p)}
                                    key={p.cronaPatientId}>
                                    <div className="patientNameCard">
                            <div className="patientName"><span style={{ backgroundColor:'lightblue' }}></span> {p.firstName + ' ' + p.lastName}</div>
